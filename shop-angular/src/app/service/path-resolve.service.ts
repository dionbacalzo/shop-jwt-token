...
export class PathResolveService implements Resolve<string | null> {
  resolve(
    ...
    let urlPath = '';
    if(state.url.startsWith('/'+paths.shop)) {
      let typoPath = state.url.replace('/'+paths.shop, '').replace('/', '');
      ...
      urlPath = `/${dictionary[0]}`;
      if(!urlPath.startsWith('/'+paths.shop)) {
        urlPath='/'+paths.shop+urlPath;
      }      
    } else {
      urlPath = '/'+paths.shop;
    }    
